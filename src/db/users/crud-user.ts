...
export async function createUser(user: User, pwd: string): Promise<User> {
	...
	await createNode(
		NodeType.USER,
		[`id:apoc.create.uuid(), email: $email, firstName: $firstName, lastName: $lastName, secondName: $secondName, auth: $auth, pwd: $pwdHash`],
		{ email: user.email, firstName: user.firstName, lastName: user.lastName, secondName: user.secondName, auth: user.auth, pwdHash },
...
export async function updateUser(emailToUpdate: string, updatedUser: UpdatedUserI): Promise<User | undefined> {
	...
	const matchedUser = await updateNode(
		NodeType.USER,
		`u`,
		`email`,
		updatedUserToProps(updatedUser),
		{ email: emailToUpdate, ...updatedUser },
		process.env.USERS_DB as string
	);
