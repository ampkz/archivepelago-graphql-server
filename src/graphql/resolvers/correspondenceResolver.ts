...
export default {
	...
	Mutation: {
		createCorrespondence: async (
			_root: any,
			{ input: { correspondenceDate, correspondenceEndDate, correspondenceType } }: any,
			{ authorizedUser }: any
		) => {
			...
			try {
				correspondence = await createCorrespondence({
					correspondenceDate: convertArchiveDateToDate(correspondenceDate),
					correspondenceEndDate: convertArchiveDateToDate(correspondenceEndDate),
					correspondenceType,
				} as ICorrespondence);
			} catch (error: any) {
		...
		updateCorrespondence: async (
			_root: any,
			{ input: { correspondenceID, updatedCorrespondenceDate, updatedCorrespondenceEndDate, updatedCorrespondenceType } }: any,
			{ authorizedUser }: any
		) => {
			if (!isPermitted(authorizedUser, Auth.ADMIN, Auth.CONTRIBUTOR)) {
				throw unauthorizedError(`You are not authorized to make this mutation.`);
			...
			try {
				correspondence = await updateCorrespondence({
					correspondenceID,
					updatedCorrespondenceDate: convertArchiveDateToDate(updatedCorrespondenceDate),
					updatedCorrespondenceType,
					updatedCorrespondenceEndDate: convertArchiveDateToDate(updatedCorrespondenceEndDate),
				});
			} catch (error: any) {
				throw mutationFailed(error.message);
			}
